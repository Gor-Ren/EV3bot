...
public class PIDIdealWallDistance implements Behavior {
	...
	public void action() {
		...
		while(!this.suppressed) {
			...
			//Control wheels
			if (output > 0) { // negative angle corresponds to pointing at wall
				RobotControls.getRightMotor().setSpeed(rightWheelMaxAngVelocity);
				RobotControls.getLeftMotor().setSpeed((int) (leftWheelMaxAngVelocity * (1 - output * 0.6))); // slow left wheel
			} else { // positive angle corresponds to pointing away from wall
				RobotControls.getRightMotor().setSpeed((int) (rightWheelMaxAngVelocity * (1 + output * 0.6))); // slow right wheel
